...
sgd_forward(images(:,:,1),filter1,filter2,filter3,w1,w2,bias1,bias2,bias3,b1,b2,labels(1));
fprintf('loss = %f \n',loss);
iterate = 0;
#t=[0];
#m=[loss];
...
while loss > 0.01,
    for m = 1 : 10,
        ...
        sgd_backward(images(:,:,m),p,labels(m)+1,max_pool_1,max_index_1,filter2,max_pool_2,max_index_2,filter3,c3,w1,w2,z1,a1);
        ...
        filter3 = filter3 - 0.01 * dfilter3;
        bias3 = bias3 - 0.01 * dbias3;
        #动量梯度下降法 卷积层2更新参数
        #vfilter2 = 0.9 * vfilter2 + 0.1 * dfilter2;
        #vbias2 = 0.9 * vbias2 + 0.1 * dbias2;
        filter2 = filter2 - 0.01 * dfilter2;
        bias2 = bias2 - 0.01 * dbias2;
        #动量梯度下降法 卷积层1更新参数
        #vfilter1 = 0.9 * vfilter1 + 0.1 * dfilter1;
        #vbias1 = 0.9 * vbias1 + 0.1 * dbias1;
        filter1 = filter1 - 0.01 * dfilter1;
        bias1 = bias1 - 0.01 * dbias1;
        #前向计算
        [c1,max_pool_1,max_index_1,c2,max_pool_2,max_index_2,c3,z1,a1,z2,p,loss] = ...
        sgd_forward(images(:,:,m),filter1,filter2,filter3,w1,w2,bias1,bias2,bias3,b1,b2,labels(m)+1);
